        ...
        <div class="card-box">
            <div class="row head_table">
                <div class="col-md-2 mr-auto mb-2">
                    <a href="{{ route('admin.aspek.index1') }}" class="btn btn-block btn-warning mt-0"><i class="mdi mdi-arrow-left"></i> Kembali</a>
                </div>
...
<script type="text/javascript">
    ...
    $(function() {
        window.table = $('#datatable').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('admin.aspek.data2',$parent->id)}}",
            },
            columns: [
                {   
                    data: 'DT_RowIndex', 
        ...
        $('#formAdd').on('submit', function(e){
            e.preventDefault();
            $.ajax({
                url: "{{ route('admin.aspek.store2',$parent->id) }}",
                type: "POST",
    ...
    function deleteData(id){
        Swal.fire({
            ...
            if(result.value){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.aspek.index2',$parent->id) }}/delete/"+id,
                    data: {
                        "id": id,
                        "_token": "{{ csrf_token() }}",
                    },
    ...
    function editData(id){
        $.ajax({
            url : "{{ route('admin.aspek.index2',$parent->id) }}/detail/"+id,
            type : "GET",
            datatype : "JSON",
